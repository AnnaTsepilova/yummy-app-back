...
const addItemInList = async (req, res) => {
    ...
    const messure = user.shopingList.filter(item => item.id.toString() === ingrId._id.toString()).filter(item => item.messure.split(' ')[1] === messureName).map(item => item.messure);
    if (checkIds.length > 0 && messure.length > 0) {
        ...
        const newIngrCount = await user.shopingList.findIndex(item => item.id.toString() === ingrToShoppingList.id.toString() && item.messure.split(' ')[1] === messureName);
