...
const removeItemFromList = async (req, res) => {
    ...
    if (recipeId) {
        const shopingListIndex = await user.shopingList.findIndex(item => item.recipesId.includes(recipeId) && item.id.toString() === shoppingListId && item.measure.trimRight() === req.query.measure);
